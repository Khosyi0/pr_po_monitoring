...
def generate_excel_export(df_plot, df_pivot, kolom_tanggal, y_col, y_label, jenis_harga, warna_map, list_resume):
    ...
    chart = LineChart()
    chart.title = create_formatted_title(f"Komparasi Tren Harga Sulfur ({jenis_harga})", is_bold=True)
    chart.height = 14
    chart.width = 32
    chart.style = None
...
def hitung_resume_sulfur(df_plot, y_col):
    """Fungsi pembantu untuk merangkai teks resume Sulfur secara otomatis"""
    ...
    poin_1 = f"Secara keseluruhan, harga Sulfur menunjukkan tren {tren} ({signifikansi}) pada {_get_nama_minggu(tgl_T0.date())}."
...
"""
            ...
            list_resume_otomatis = hitung_resume_sulfur(df_plot, y_col)
            st.markdown("##### *Resume :*")
