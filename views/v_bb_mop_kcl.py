...
def generate_excel_export(df_plot, df_pivot, kolom_tanggal, y_col, y_label, jenis_harga, warna_map, list_resume):
    wb = Workbook()
    ws = wb.active
    ws.title = "Komparasi Harga MOP-KCl"
...
def hitung_resume_mop_kcl(df_plot, y_col):
    """Fungsi pembantu untuk merangkai teks resume MOP-KCl secara otomatis (3 Titik Waktu)"""
    ...
    threshold_signifikan = 15.0 # Batas untuk MOP-KCl
    ...
    if delta_recent < -2.0:
        ...
        konteks_historis = f" setelah sempat naik sepanjang {bulan_indo[tgl_T1.month]}" if delta_past > 2.0 else ""
    elif delta_recent > 2.0:
        tren_sekarang = "menunjukkan tren meningkat"
        signifikansi = " namun tidak signifikan" if abs(delta_recent) < threshold_signifikan else " yang cukup signifikan"
        konteks_historis = f" setelah sempat turun sepanjang {bulan_indo[tgl_T1.month]}" if delta_past < -2.0 else ""
    else:
        tren_sekarang = "terpantau stabil"
        signifikansi, konteks_historis = "", ""

    poin_1 = f"Secara keseluruhan, harga MOP-KCl {tren_sekarang}{signifikansi} pada {_get_nama_minggu(tgl_T0.date())}{konteks_historis}."

    if tren_sekarang == "terpantau stabil":
        poin_2 = f"Harga bertahan pada tingkat kestabilan konstan jika dibandingkan dengan {_get_nama_minggu(tgl_T2.date())} yang berada di rata - rata USD {harga_T2:.2f}/MT."
    else:
        arah = "Kenaikan" if delta_recent > 2.0 else "Penurunan"
        poin_2 = f"Terdapat {arah.lower()} harga, namun secara posisi rata-rata saat ini (USD {harga_T0:.2f}/MT) terpaut {abs(harga_T0 - harga_T2):.2f} USD/MT jika dibandingkan dengan periode {_get_nama_minggu(tgl_T2.date())}."
...
def render(load_data, global_context):
    st.markdown("### :material/science: Analisis Tren Komparasi Harga Pasar: MOP-KCl")
    ...
    query = """
        ...
        WHERE bahan_baku = 'MOP-KCl'
        ORDER BY tanggal_terbit ASC
    """
    df = load_data(query)

    if df.empty:
        st.warning("Data harga MOP-KCl belum tersedia di database.")
        return
    ...
    with st.expander(":material/settings: Filter Komparasi Harga Pasar", expanded=True):
        ...
        with col_mulai:
            start_date = st.date_input(
                "Mulai dari tanggal",
                value=st.session_state.get("_perm_start_date_mop_kcl", default_start_date),
                min_value=calendar_min_date,
                max_value=max_date,
                key="start_date_mop_kcl",
                on_change=_save_to_permanent,
                args=("start_date_mop_kcl", "_perm_start_date_mop_kcl")
            )
        with col_sampai:
            end_date = st.date_input(
                "Sampai tanggal",
                value=st.session_state.get("_perm_end_date_mop_kcl", max_date),
                min_value=calendar_min_date,
                max_value=max_date,
                key="end_date_mop_kcl",
                on_change=_save_to_permanent,
                args=("end_date_mop_kcl", "_perm_end_date_mop_kcl")
            )
        with col_metode:
            jenis_harga_options = ["AVERAGE", "MIN", "MAX"]
            jenis_harga_default = st.session_state.get("_perm_jenis_harga_mop_kcl", "AVERAGE")
            jenis_harga = st.selectbox(
                ...
                key="jenis_harga_mop_kcl",
                on_change=_save_to_permanent,
                args=("jenis_harga_mop_kcl", "_perm_jenis_harga_mop_kcl")
            )
        with col_jml:
            jml_komparasi = st.number_input(
                "Jumlah Komparasi",
                min_value=1, max_value=5,
                value=st.session_state.get("_perm_jml_komparasi_mop_kcl", 2),
                key="jml_komparasi_mop_kcl",
                on_change=_save_to_permanent,
                args=("jml_komparasi_mop_kcl", "_perm_jml_komparasi_mop_kcl")
            )
        ...
        for i in range(int(jml_komparasi)):
            ...
            with c1:
                perm_key_majalah = f"_perm_majalah_mop_kcl_{i}"
                ...
                majalah_pilihan = st.selectbox(
                    ...
                    index=majalah_index,
                    key=f"majalah_mop_kcl_{i}",
                    on_change=_save_to_permanent,
                    args=(f"majalah_mop_kcl_{i}", perm_key_majalah)
                )
            with c2:
                list_incoterm = df[df['nama_majalah'] == majalah_pilihan]['incoterm'].unique()
                perm_key_incoterm = f"_perm_incoterm_mop_kcl_{i}"
                ...
                incoterm_pilihan = st.selectbox(
                    ...
                    key=f"incoterm_mop_kcl_{i}",
                    on_change=_save_to_permanent,
                    args=(f"incoterm_mop_kcl_{i}", perm_key_incoterm)
                )
            with c3:
                perm_key_warna = f"_perm_color_mop_kcl_{i}"
                default_warna = st.session_state.get(perm_key_warna, default_colors[i % len(default_colors)])
                warna_pilihan = st.color_picker(
                    "Warna", default_warna,
                    key=f"color_mop_kcl_{i}",
                    on_change=_save_to_permanent,
                    args=(f"color_mop_kcl_{i}", perm_key_warna)
                )
    ...
    if start_date <= end_date and komparasi_data:
        ...
        if not df_plot.empty:
            ...
            fig = px.line(
                ...
                title=f"Komparasi Tren Harga MOP-KCl ({jenis_harga})",
...
"""
            ...
            list_resume_otomatis = hitung_resume_mop_kcl(df_plot, y_col)
            ...
            st.download_button(
                ...
                data=excel_buffer,
                file_name=f"komparasi_harga_mop_kcl_{jenis_harga}_{start_date}_{end_date}.xlsx",
