...
def generate_excel_export(df_plot, df_pivot, kolom_tanggal, y_col, y_label, jenis_harga, warna_map, list_resume):
    ...
    chart = LineChart()
    chart.title = create_formatted_title(f"Komparasi Tren Harga Phosphate Rock ({jenis_harga})", is_bold=True)
    chart.height = 14
    chart.width = 32
    chart.style = None
    ...
    with st.expander(":material/settings: Filter Komparasi Harga Pasar", expanded=True):
        ...
        with col_mulai:
            start_date = st.date_input(
                "Mulai dari tanggal",
                value=st.session_state.get("_perm_start_date_phosrock", default_start_date),
                min_value=calendar_min_date, max_value=max_date,
                key="start_date_phosrock",
                on_change=_save_to_permanent,
                args=("start_date_phosrock", "_perm_start_date_phosrock")
            )
        with col_sampai:
            end_date = st.date_input("Sampai tanggal", value=st.session_state.get("_perm_end_date_phosrock", max_date),
                                      min_value=calendar_min_date, max_value=max_date,
                                      key="end_date_phosrock",
                                      on_change=_save_to_permanent,
                                      args=("end_date_phosrock", "_perm_end_date_phosrock"))
        with col_metode:
            jenis_harga_options = ["AVERAGE", "MIN", "MAX"]
            jenis_harga_default = st.session_state.get("_perm_jenis_harga_phosrock", "AVERAGE")
            jenis_harga = st.selectbox(
                ...
                key="jenis_harga_phosrock",
                on_change=_save_to_permanent,
                args=("jenis_harga_phosrock", "_perm_jenis_harga_phosrock")
            )
        with col_jml:
            jml_komparasi = st.number_input(
                "Jumlah Komparasi",
                min_value=1, max_value=5,
                value=st.session_state.get("_perm_jml_komparasi_phosrock", 2),
                key="jml_komparasi_phosrock",
                on_change=_save_to_permanent,
                args=("jml_komparasi_phosrock", "_perm_jml_komparasi_phosrock")
            )
        ...
        for i in range(int(jml_komparasi)):
            ...
            with c1:
                perm_key_majalah = f"_perm_majalah_phosrock_{i}"
                ...
                majalah_pilihan = st.selectbox(
                    ...
                    index=majalah_index,
                    key=f"majalah_phosrock_{i}",
                    on_change=_save_to_permanent,
                    args=(f"majalah_phosrock_{i}", perm_key_majalah))
            with c2:
                list_incoterm = df[df['nama_majalah'] == majalah_pilihan]['incoterm'].unique()
                perm_key_incoterm = f"_perm_incoterm_phosrock_{i}"
                ...
                incoterm_pilihan = st.selectbox(
                    ...
                    on_change=_save_to_permanent,
                    args=(f"incoterm_phosrock_{i}", perm_key_incoterm))
            with c3:
                perm_key_warna = f"_perm_color_phosrock_{i}"
                default_warna = st.session_state.get(perm_key_warna, default_colors[i % len(default_colors)])
                warna_pilihan = st.color_picker(
                    "Warna", default_warna,
                    key=f"color_phosrock_{i}",
                    on_change=_save_to_permanent,
                    args=(f"color_phosrock_{i}", perm_key_warna))
