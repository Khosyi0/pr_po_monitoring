...
def generate_excel_export(df_plot, df_pivot, kolom_tanggal, y_col, y_label, jenis_harga, warna_map, list_resume):
    ...
    for idx, poin in enumerate(list_resume, start=1):
        ...
        # Kalkulasi tinggi baris otomatis agar teks panjang tidak terpotong
...
def hitung_resume_urea(df_plot, y_col):
    """Fungsi pembantu untuk merangkai teks resume Urea secara otomatis"""
    ...
    poin_1 = f"Secara keseluruhan, harga Urea menunjukkan tren {tren} ({signifikansi}) pada {_get_nama_minggu(tgl_T0.date())}."
...
def render(load_data, global_context):
    st.markdown("### :material/science: Analisis Tren Komparasi Harga Pasar: Urea")
    ...
    query = """
        ...
        WHERE bahan_baku = 'Urea'
        ORDER BY tanggal_terbit ASC
    """
    df = load_data(query)

    if df.empty:
        st.warning("Data harga Urea belum tersedia di database.")
        return
    ...
    with st.expander(":material/settings: Filter Komparasi Harga Pasar", expanded=True):
        ...
        with col_mulai:
            start_date = st.date_input(
                "Mulai dari tanggal",
                value=st.session_state.get("_perm_start_date_urea", default_start_date),
                min_value=calendar_min_date,
                max_value=max_date,
                key="start_date_urea",
                on_change=_save_to_permanent,
                args=("start_date_urea", "_perm_start_date_urea")
            )
        with col_sampai:
            end_date = st.date_input(
                "Sampai tanggal",
                value=st.session_state.get("_perm_end_date_urea", max_date),
                min_value=calendar_min_date,
                max_value=max_date,
                key="end_date_urea",
                on_change=_save_to_permanent,
                args=("end_date_urea", "_perm_end_date_urea")
            )
        with col_metode:
            jenis_harga_options = ["AVERAGE", "MIN", "MAX"]
            jenis_harga_default = st.session_state.get("_perm_jenis_harga_urea", "AVERAGE")
            jenis_harga = st.selectbox(
                ...
                key="jenis_harga_urea",
                on_change=_save_to_permanent,
                args=("jenis_harga_urea", "_perm_jenis_harga_urea")
            )
        with col_jml:
            jml_komparasi = st.number_input(
                "Jumlah Komparasi",
                min_value=1, max_value=5,
                value=st.session_state.get("_perm_jml_komparasi_urea", 2),
                key="jml_komparasi_urea",
                on_change=_save_to_permanent,
                args=("jml_komparasi_urea", "_perm_jml_komparasi_urea")
            )
        ...
        for i in range(int(jml_komparasi)):
            ...
            with c1:
                perm_key_majalah = f"_perm_majalah_urea_{i}"
                ...
                majalah_pilihan = st.selectbox(
                    ...
                    index=majalah_index,
                    key=f"majalah_urea_{i}",
                    on_change=_save_to_permanent,
                    args=(f"majalah_urea_{i}", perm_key_majalah)
                )
            with c2:
                list_incoterm = df[df['nama_majalah'] == majalah_pilihan]['incoterm'].unique()
                perm_key_incoterm = f"_perm_incoterm_urea_{i}"
                ...
                incoterm_pilihan = st.selectbox(
                    ...
                    key=f"incoterm_urea_{i}",
                    on_change=_save_to_permanent,
                    args=(f"incoterm_urea_{i}", perm_key_incoterm)
                )
            with c3:
                perm_key_warna = f"_perm_color_urea_{i}"
                default_warna = st.session_state.get(perm_key_warna, default_colors[i % len(default_colors)])
                warna_pilihan = st.color_picker(
                    "Warna", default_warna,
                    key=f"color_urea_{i}",
                    on_change=_save_to_permanent,
                    args=(f"color_urea_{i}", perm_key_warna)
                )
    ...
    if start_date <= end_date and komparasi_data:
        ...
        if not df_plot.empty:
            ...
            fig = px.line(
                ...
                title=f"Komparasi Tren Harga Urea ({jenis_harga})",
...
"""
            ...
            list_resume_otomatis = hitung_resume_urea(df_plot, y_col)
            st.markdown("##### *Resume :*")
            ...
            st.download_button(
                ...
                data=excel_buffer,
                file_name=f"komparasi_harga_Urea_{jenis_harga}_{start_date}_{end_date}.xlsx",
