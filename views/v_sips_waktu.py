...
def render(load_data, date_from, date_to, selected_nama, selected_bagian=None, **kwargs):
    ...
    if no_data:
        ...
        # Tampilkan struktur section chart tanpa data
        _empty_sections = [
            "KPI Ringkasan Waktu Proses",
            "Dekomposisi Waktu per Karyawan",
            "Pemenuhan SLA per Jenis Pengadaan",
            "SLA Headroom per Karyawan",
            "Tren Waktu Proses per Bulan",
            "Distribusi Waktu Proses",
            "Waktu Proses per Prioritas",
            "Waktu Realisasi SLA per Purchasing Group",
        ]
        for _sec in _empty_sections:
            st.markdown(f"<h3 style='font-size:22px; margin-bottom:4px'>{_sec}</h3>",
    ...
    **Distribusi Waktu**
    ...
    """)
        ...
        with col_r:
            ...
            # Mengubah data yang diplot menjadi realisasi_sla
    ...
    """)
        ...
        with title_col:
            st.markdown("""
                ...
                <p style='opacity:.55; font-size:14px; margin:-10px 0 10px 0;'>Perbandingan rata-rata waktu penyelesaian berdasarkan grup purchasing</p>
            """, unsafe_allow_html=True)
        with btn_col:
            st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
    ...
    **Kalkulasi SQL:**
    ...
    """)
        ...
        if "purchasing_group" in df.columns:
