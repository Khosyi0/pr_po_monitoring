            ...
            with st.chat_message("assistant", avatar="assets/Melati_icon.png"):
                with st.spinner("Tunggu, Melati sedang menganalisis data..."):
                    try:
                        ...
                        3a. KAITAN ISU DENGAN HARGA: Jika BUKTI DATA menyertakan bagian "ISU TERKAIT", kamu boleh menyebutkan isu tersebut sebagai kemungkinan konteks/latar belakang di balik pergerakan harga, TAPI kamu WAJIB bersikap hati-hati: sebutkan itu sebagai "kemungkinan berkaitan" atau "berdasarkan catatan isu", bukan sebagai kepastian sebab-akibat, kecuali isu tersebut secara eksplisit menyebut penyebabnya. Jika tidak ada isu yang relevan pada rentang tanggal yang ditanyakan, katakan dengan jujur bahwa tidak ada catatan isu terkait, jangan mengarang penjelasan.Untuk isu yang ditandai "MASIH BERLANGSUNG", perlakukan sebagai kondisi latar belakang jangka panjang -- jangan otomatis mengklaim isu tersebut sebagai penyebab SETIAP fluktuasi harga di sepanjang periode setelahnya, kecuali ada lonjakan/perubahan tren yang jelas bertepatan dengan tanggal mulainya.
                        3b. EVALUASI APAKAH DAMPAK ISU MASIH TERASA: Jika user bertanya apakah dampak suatu isu "masih terasa/berlangsung sampai sekarang", kamu WAJIB menjawab dengan cara MEMBANDINGKAN ANGKA, bukan asumsi:
                            a. Cari harga pada/sekitar tanggal kejadian isu tersebut (baseline "saat isu terjadi") dari data komparasi harga yang tersedia.
                            b. Bandingkan dengan harga TERKINI (tanggal data paling akhir yang tersedia) untuk bahan baku dan referensi yang sama.
                            c. Jika data yang tersedia (sesuai rentang filter yang sedang aktif) TIDAK mencakup periode sebelum/saat isu terjadi, kamu WAJIB mengatakan dengan jujur bahwa kamu tidak bisa menyimpulkan karena datanya tidak mencakup periode tersebut -- jangan menebak.
                            d. Jawaban harus SPESIFIK PER REFERENSI/MAJALAH-INCOTERM, karena satu bahan baku bisa punya beberapa referensi harga dengan pola berbeda -- jangan menggeneralisasi satu kesimpulan untuk semua referensi jika datanya menunjukkan pola berbeda (misal satu referensi sudah kembali ke level sebelum isu, sementara referensi lain masih tinggi).
                            e. Gunakan bahasa yang mencerminkan ketidakpastian yang wajar, misal "berdasarkan data yang tersedia, terlihat bahwa..." bukan klaim mutlak, karena kenaikan/penurunan harga bisa dipengaruhi banyak faktor selain isu yang disebut.
